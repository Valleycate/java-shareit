...
public class HandleBookingStateFutureWithItems extends HandlerBookingState {
    private final ItemServiceImpl itemService;

    public HandleBookingStateFutureWithItems(BookingDbRepository repository, BookingMapper mapper, ItemServiceImpl itemService) {
    ...
    public List<BookingDtoAnswer> findBookings(int userId, int page, int size) {
        ...
        itemService.findAllItemsByUserForBooking(userId)
                .forEach(itemDto -> ans.addAll(repository.findByItem_IdAndStartIsAfterOrderByStartDesc(
                                itemDto.getId(), LocalDateTime.now(), PageRequest.of(page, size)).stream()
