...
public class HandleBookingStateWaitingWithItems extends HandlerBookingState {
    private final ItemServiceImpl itemService;

    public HandleBookingStateWaitingWithItems(BookingDbRepository repository, BookingMapper mapper, ItemServiceImpl itemService) {
    ...
    public List<BookingDtoAnswer> findBookings(int userId, int page, int size) {
        ...
        itemService.findAllItemsByUserForBooking(userId)
                .forEach(itemDto -> ans.addAll(repository.findByItem_IdAndStatusOrderByStartDesc(itemDto.getId(),
                                BookingStatus.WAITING, PageRequest.of(page, size)).stream()
